...
--WHERE절에서 PUBLISHER, PRICE 동시 사용시 IX_BOOK2 인덱스 적용
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE = 8000;

--인덱스가 있는데 사용이 안되는 경우
SELECT * FROM BOOK WHERE PUBLISHER LIKE '대한%';  --인덱스 사용(구간검색
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; --인덱스 적용X(전체비교)
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; --인덱스 적용X(전체비교)

--원본 데이터를 가공하는 경우(인덱스 정용X)
